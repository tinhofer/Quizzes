...
// Wählt zufällig Fragen aus und stellt sicher, dass sie in zufälliger Reihenfolge erscheinen
function selectQuestions() {
    // Anzahl der zu stellenden Fragen (anpassbar)
    const questionsCount = 10;
    
    // Zufällige Auswahl von Fragen ohne Berücksichtigung früherer Quizze
    // Fisher-Yates (Knuth) Shuffle-Algorithmus für eine faire Zufallsauswahl
    ...
    // Die ersten 10 Fragen auswählen (oder weniger, falls nicht genügend Fragen verfügbar)
    return shuffledQuestions.slice(0, Math.min(questionsCount, shuffledQuestions.length));
}
...
function showQuizSummary() {
    ...
    // Neustart-Button, der die restartQuiz-Funktion aufruft statt die Seite neu zu laden
    buttonContainer.innerHTML = '<button id="restart-button">Quiz neu starten</button>';
...
// Quiz neustarten mit neuen, zufälligen Fragen
function restartQuiz() {
    ...
    // Neue Fragen in zufälliger Reihenfolge auswählen
    const questions = selectQuestions();
    
    // UI zurücksetzen
    quizTitle.innerText = 'KI und Arbeitsrecht Quiz';
    buttonContainer.innerHTML = originalButtonContainerHTML;
    scoreContainer.innerHTML = originalScoreContainerHTML;
...
// Fragen in zufälliger Reihenfolge auswählen und Quiz starten
allQuestions = questions; // Speichere alle verfügbaren Fragen
questions = selectQuestions(); // Wähle zufällige Fragen für dieses Quiz aus
loadQuestion();